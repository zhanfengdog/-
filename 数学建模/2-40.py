...
iris_target=np.float32(tf.keras.utils.to_categorical(data.target,
                                                     num_classes=3))     
train_data=tf.data.Dataset.from_tensor_slices(((iris_data_1,
                                                iris_data_2),
                                               (iris_target,iris_label))).batch(128)
...
model=tf.keras.Model(inputs=[input_xs_1,input_xs_2],outputs=[logits,label])#改了
...
model.compile(optimizer=tf.optimizers.Adam(1e-3),
              loss={'predictions':tf.losses.categorical_crossentropy,
                    'label':my_MSE},loss_weights={'predictions':0.1,'label':0.5},
              metrics=['accuracy'])
